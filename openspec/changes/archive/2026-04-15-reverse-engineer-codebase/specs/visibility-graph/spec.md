...
Both `searchAStar` and `searchDijkstra` SHALL return a `Promise<number[][]>` where each element is a `[longitude, latitude]` pair. The array SHALL be ordered from start to target. An empty array SHALL be returned when no path is found within `distanceMax`.

#### Scenario: Path found

- **WHEN** `searchAStar` or `searchDijkstra` is called and a route exists within `distanceMax`
...
Both search methods SHALL yield to the Node.js event loop at least once every 10 search iterations by awaiting a `setTimeout(0)` promise, ensuring the service remains responsive during long-running searches.
